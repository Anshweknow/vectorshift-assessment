...
export const SubmitButton = () => {
    ...
    const analysis = result ? normalizeAnalysisResult(result, nodes, edges) : null;
            ...
            {analysis && (
                <div className="analysis-card" role="status" aria-live="polite">
                    <div className="analysis-card__header">
                        ...
                        <span className={`status-badge ${analysis.is_dag ? 'status-badge--success' : 'status-badge--danger'}`}>
                            {analysis.is_dag ? 'DAG Valid' : 'Cycle Detected'}
                        </span>
                    </div>
                    <div className="analysis-grid">
                        <div className="metric-card">
                            <span className="metric-card__label">Total Nodes</span>
                            <span className="metric-card__value">{analysis.num_nodes}</span>
                        </div>
                        <div className="metric-card">
                            <span className="metric-card__label">Total Edges</span>
                            <span className="metric-card__value">{analysis.num_edges}</span>
                        </div>
                        <div className="metric-card">
                            <span className="metric-card__label">DAG Status</span>
                            <span className="metric-card__value">{analysis.is_dag ? 'Yes' : 'No'}</span>
                        </div>
                    </div>
                </div>
            )}
